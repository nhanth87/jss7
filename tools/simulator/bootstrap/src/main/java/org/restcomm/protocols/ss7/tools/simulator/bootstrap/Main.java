...
public class Main {
    ...
    public static void main(String[] args) throws Throwable {
        ...
        if (!initLOG4JProperties(homeDir) && !initLOG4JXml(homeDir)) {
            logger.error("Failed to initialize logging, no available configuration. Defaults shall be used.");
        }
    ...
    protected static boolean initLOG4JXml(String homeDir) {
        ...
        try {
            URL log4jurl = getURL(Log4jURL);
            // log4j2 auto-loads its configuration from the classpath (log4j2.xml);
            // here we only verify the legacy config file is reachable.
            log4jurl.openStream().close();
        } catch (Exception e) {
            // e.printStackTrace();
