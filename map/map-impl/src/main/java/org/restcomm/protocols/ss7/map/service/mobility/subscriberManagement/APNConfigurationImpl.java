...
public class APNConfigurationImpl extends SequenceBase implements APNConfiguration {
    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws MAPException {
        ...
        try {
            ...
            asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _TAG_contextId, this.contextId);

            ((PDNTypeImpl) this.pDNType).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC, _TAG_pDNType);

            if (this.servedPartyIPIPv4Address != null)
                ((PDPAddressImpl) this.servedPartyIPIPv4Address).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC,
                        _TAG_servedPartyIPIPv4Address);

            ((APNImpl) this.apn).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC, _TAG_apn);

            ((EPSQoSSubscribedImpl) this.ePSQoSSubscribed).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC, _TAG_ePSQoSSubscribed);
            ...
            if (this.pdnGwAllocationType != null)
                asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _TAG_pdnGwAllocationType, this.pdnGwAllocationType.getCode());

            if (vplmnAddressAllowed)
                asnOutputStream.writeNull(Tag.CLASS_CONTEXT_SPECIFIC, _TAG_vplmnAddressAllowed);
            ...
            if (specificAPNInfoList != null) {
                asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _TAG_specificAPNInfoList);
                int pos = asnOutputStream.StartContentDefiniteLength();
                for (SpecificAPNInfo specificAPNInfo : this.specificAPNInfoList) {
                    ((SpecificAPNInfoImpl) specificAPNInfo).encodeAll(asnOutputStream);
                }
            ...
            if (this.servedPartyIPIPv6Address != null)
                ((PDPAddressImpl) this.servedPartyIPIPv6Address).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC,
            ...
            if (this.siptoPermission != null)
                asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _TAG_siptoPermission, this.siptoPermission.getCode());

            if (this.lipaPermission != null)
                asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _TAG_lipaPermission, this.lipaPermission.getCode());
