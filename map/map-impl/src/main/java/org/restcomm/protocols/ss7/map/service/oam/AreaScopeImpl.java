    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws MAPException {
        try {
            if (this.cgiList != null) {
                asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _ID_cgiList);
                int pos = asnOutputStream.StartContentDefiniteLength();
                for (GlobalCellId globalCellId : this.cgiList) {
                    ((GlobalCellIdImpl) globalCellId).encodeAll(asnOutputStream);
                }
                asnOutputStream.FinalizeContent(pos);
            }
            if (this.eUtranCgiList != null) {
                asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _ID_eUtranCgiList);
                int pos = asnOutputStream.StartContentDefiniteLength();
                for (EUtranCgi eUtranCgi : this.eUtranCgiList) {
                    ((EUtranCgiImpl) eUtranCgi).encodeAll(asnOutputStream);
                }
                asnOutputStream.FinalizeContent(pos);
            }
            if (this.routingAreaIdList != null) {
                asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _ID_routingAreaIdList);
                int pos = asnOutputStream.StartContentDefiniteLength();
                for (RAIdentity raIdentity : this.routingAreaIdList) {
                    ((RAIdentityImpl) raIdentity).encodeAll(asnOutputStream);
                }
                asnOutputStream.FinalizeContent(pos);
            }
            if (this.locationAreaIdList != null) {
                asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _ID_locationAreaIdList);
                int pos = asnOutputStream.StartContentDefiniteLength();
                for (LAIFixedLength laiFixedLength : this.locationAreaIdList) {
                    ((LAIFixedLengthImpl) laiFixedLength).encodeAll(asnOutputStream);
                }
                asnOutputStream.FinalizeContent(pos);
            }
            if (this.trackingAreaIdList != null) {
                asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _ID_trackingAreaIdList);
                int pos = asnOutputStream.StartContentDefiniteLength();
                for (TAId taId : this.trackingAreaIdList) {
                    ((TAIdImpl) taId).encodeAll(asnOutputStream);
                }
                asnOutputStream.FinalizeContent(pos);
            }
