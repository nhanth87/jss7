...
public class SendRoutingInfoForSMResponseImpl extends SmsMessageImpl implements SendRoutingInfoForSMResponse {
    ...
    private void _decodeDispatch(AsnInputStream asnInputStream, int length)
        ...
        if (MapBerSupport.useBer(asnInputStream)) {
            try {
                ...
                this.imsi = null; this.locationInfoWithLMSI = null;
            }
        }
    ...
    private void _decodeBer(byte[] buf, int offset, int length)
        ...
        if (!c.hasMore()) throw new AsnException(_PrimitiveName + ": missing imsi");
        c.readTag();
        if (c.tagClass() != BerTag.UNIVERSAL || !c.isPrimitive() || c.tag() != BerTag.OCTET_STRING)
            throw new AsnException(_PrimitiveName + ": bad imsi");
        IMSIImpl im = new IMSIImpl();
        im.decodeData(MapBerSupport.taggedValueStream(c), c.valueLength());
        this.imsi = im; c.skipValue();

        if (!c.hasMore()) throw new AsnException(_PrimitiveName + ": missing locationInfoWithLMSI");
        c.readTag();
        if (c.tagClass() != BerTag.CONTEXT || c.isPrimitive() || c.tag() != _TAG_LocationInfoWithLMSI)
            throw new AsnException(_PrimitiveName + ": bad locationInfoWithLMSI");
        LocationInfoWithLMSIImpl loc = new LocationInfoWithLMSIImpl();
        loc.decodeData(MapBerSupport.taggedValueStream(c), c.valueLength());
        this.locationInfoWithLMSI = loc; c.skipValue();
