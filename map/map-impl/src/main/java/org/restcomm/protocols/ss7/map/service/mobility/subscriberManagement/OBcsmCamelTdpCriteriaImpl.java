...
public class OBcsmCamelTdpCriteriaImpl extends SequenceBase implements OBcsmCamelTdpCriteria {
    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws MAPException {
        ...
        try {
            ...
            asnOutputStream.writeInteger(Tag.CLASS_UNIVERSAL, Tag.ENUMERATED, this.oBcsmTriggerDetectionPoint.getCode());

            if (this.destinationNumberCriteria != null)
                ((DestinationNumberCriteriaImpl) this.destinationNumberCriteria).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC,
                        _TAG_destinationNumberCriteria);

            if (this.basicServiceCriteria != null) {
                asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _TAG_basicServiceCriteria);
            ...
            if (this.callTypeCriteria != null)
                asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _TAG_callTypeCriteria, this.callTypeCriteria.getCode());

            if (this.oCauseValueCriteria != null) {
                asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _TAG_oCauseValueCriteria);
                int pos = asnOutputStream.StartContentDefiniteLength();
                for (CauseValue causeValue : this.oCauseValueCriteria) {
                    ((CauseValueImpl) causeValue).encodeAll(asnOutputStream);
                }
                asnOutputStream.FinalizeContent(pos);
            }
