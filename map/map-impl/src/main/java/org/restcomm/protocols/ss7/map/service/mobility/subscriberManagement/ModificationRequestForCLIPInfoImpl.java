 ...
 */
public class ModificationRequestForCLIPInfoImpl extends SequenceBase implements ModificationRequestForCLIPInfo {

  private static final int _TAG_SS_STATUS = 0;
  private static final int _TAG_OVERRIDE_CATEGORY = 1;
  private static final int _TAG_MODIFY_NOTIFICATION_TO_CSE = 2;
  private static final int _TAG_EXTENSION_CONTAINER = 3;

  public static final String _PrimitiveName = "ModificationRequestForCLIPInfo";
  ...
  public ModificationRequestForCLIPInfoImpl(ExtSSStatus extSSStatus, OverrideCategory overrideCategory,
      ...
      if (ais.getTagClass() == Tag.CLASS_CONTEXT_SPECIFIC) {
        switch (tag) {
          ...
          case _TAG_OVERRIDE_CATEGORY:
            ...
            if (!ais.isTagPrimitive())
              throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                  + ".overrideCategory: parameter is not primitive", MAPParsingComponentExceptionReason.MistypedParameter);
    ...
    try {
      ...
      if (this.overrideCategory != null) {
        try {
          ...
          throw new MAPException("IOException while encoding " + _PrimitiveName + " parameter overrideCategory", e);
        } catch (AsnException e) {
          throw new MAPException("AsnException while encoding " + _PrimitiveName + " parameter overrideCategory", e);
        }
      }
