...
public class GSMCharset extends Charset {
    ...
    public GSMCharset(String canonicalName, String[] aliases) {
        this(canonicalName, aliases, BYTE_TO_CHAR_DefaultAlphabet, BYTE_TO_CHAR_DefaultAlphabetExtensionTable);
    }
    ...
    public GSMCharset(String canonicalName, String[] aliases,
        ...
        if (nationalLanguageSingleShiftIdentifier == null) {
            this.extensionTable = BYTE_TO_CHAR_DefaultAlphabetExtensionTable;
        } else
            switch (nationalLanguageSingleShiftIdentifier) {
                case Urdu:
                    this.extensionTable = BYTE_TO_CHAR_UrduAlphabetExtensionTable;
                    break;
                default:
                    this.extensionTable = BYTE_TO_CHAR_DefaultAlphabetExtensionTable;
                    break;
     ...
     * @return
     */
    public static int checkEncodedDataLengthInChars(String data, int[] mainTable, int[] extensionTable) {
        if (data == null)
            return 0;
        ...
        for (int i1 = 0; i1 < data.length(); i1++) {
            ...
            if (!found && extensionTable != null) {
                for (int i = 0; i < extensionTable.length; i++) {
                    if (c != 0 && extensionTable[i] == c) {
                        found = true;
                        cnt += 2;
                        break;
                    }
     ...
     * @return
     */
    public static int septetsToOctets(int septetsCount) {
        int byteCnt = (septetsCount + 1) * 7 / 8;
        return byteCnt;
    }
     ...
     * @return
     */
    public static int octetsToSeptets(int byteCount) {
        int septCnt = (byteCount * 8 - 1) / 7 + 1;
        return septCnt;
    }
     ...
     * @return
     */
    public static String[] sliceString(String data, int charCount, int[] mainTable, int[] extensionTable) {
        if (data == null)
            return null;
    ...
    public static final int[] BYTE_TO_CHAR_DefaultAlphabetExtensionTable = { 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    ...
    public static final int[] BYTE_TO_CHAR_UrduAlphabetExtensionTable = { 0x0040, 0x00A3, 0x0024, 0x00A5, 0x00BF, 0x0022,
