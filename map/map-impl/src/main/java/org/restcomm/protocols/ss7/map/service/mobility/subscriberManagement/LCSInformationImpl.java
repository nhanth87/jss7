    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws MAPException {
        ...
        try {
            ...
            if (this.gmlcList != null) {
                asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _TAG_gmlcList);
            ...
            if (this.lcsPrivacyExceptionList != null) {
                asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _TAG_lcsPrivacyExceptionList);
                int pos = asnOutputStream.StartContentDefiniteLength();
                for (LCSPrivacyClass lcsPrivacyClass : this.lcsPrivacyExceptionList) {
                    ((LCSPrivacyClassImpl) lcsPrivacyClass).encodeAll(asnOutputStream);
                }
                asnOutputStream.FinalizeContent(pos);
            }

            if (this.molrList != null) {
                asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _TAG_molrList);
                int pos = asnOutputStream.StartContentDefiniteLength();
                for (MOLRClass molrClass : this.molrList) {
                    ((MOLRClassImpl) molrClass).encodeAll(asnOutputStream);
                }
                asnOutputStream.FinalizeContent(pos);
            }

            if (this.addLcsPrivacyExceptionList != null) {
                asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, _TAG_addLcsPrivacyExceptionList);
                int pos = asnOutputStream.StartContentDefiniteLength();
