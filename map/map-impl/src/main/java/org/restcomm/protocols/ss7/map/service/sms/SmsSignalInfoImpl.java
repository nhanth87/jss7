...
public class SmsSignalInfoImpl implements SmsSignalInfo, MAPAsnPrimitive {
    ...
    /**
     * Copies {@code length} bytes starting at {@code offset} from {@code backing} into
     * this TPDU. Used by the BerCursor decode path, which navigates a snapshot of the
     * inbound buffer; the value is copied out so it does not alias that buffer.
     */
    public void setData(byte[] backing, int offset, int length) {
        this.data = java.util.Arrays.copyOfRange(backing, offset, offset + length);
    }
    ...
    public SmsTpdu decodeTpdu(boolean mobileOriginatedMessage) throws MAPException {
        return SmsTpduImpl.createInstance(this.getData(), mobileOriginatedMessage, this.getGsm8Charset());
    }
    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws MAPException {
        ...
        asnOutputStream.writeOctetStringData(d);
    }
    ...
    public String toString() {
        ...
        try {
            SmsTpdu tpdu = SmsTpduImpl.createInstance(d, true, getGsm8Charset());
            sb.append("MO case: ");
            sb.append(tpdu);
            moExists = true;
        ...
        try {
            if (moExists)
                sb.append("\n");
            SmsTpdu tpdu = SmsTpduImpl.createInstance(d, false, getGsm8Charset());
            sb.append("MT case: ");
            sb.append(tpdu);
        } catch (MAPException e) {
            logger.error(e.getMessage());
