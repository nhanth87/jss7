...
public class SendRoutingInformationRequestImpl extends CallHandlingMessageImpl implements SendRoutingInformationRequest {
    ...
    private void _decodeBer(byte[] buf, int offset, int length)
        ...
        while (c.hasMore()) {
            ...
            switch (c.tag()) {
                case TAG_msisdn -> {
                    ...
                    MapBerSupport.decodeNested(c, m);
                    this.msisdn = m;
                }
                ...
                case TAG_numberOfForwarding -> {
                    if (!c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": numberOfForwarding is not primitive");
                    this.numberOfForwarding = MapBerSupport.readInt(c);
                }
                case TAG_interrogationType -> {
                    if (!c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": interrogationType is not primitive");
                    this.interrogationType = InterrogationType.getInterrogationType(MapBerSupport.readInt(c));
                }
                ...
                case TAG_orCapability -> {
                    if (!c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": orCapability is not primitive");
                    this.orCapability = MapBerSupport.readInt(c);
                }
                case TAG_gmscOrGsmSCFAddress -> {
                    if (!c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": gmscAddress is not primitive");
                    ISDNAddressStringImpl gmsc = new ISDNAddressStringImpl();
                    MapBerSupport.decodeNested(c, gmsc);
                    this.gmscAddress = gmsc;
                }
                case TAG_callReferenceNumber -> {
                    if (!c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": callReferenceNumber is not primitive");
                    CallReferenceNumberImpl crn = new CallReferenceNumberImpl();
                    MapBerSupport.decodeNested(c, crn);
                    this.callReferenceNumber = crn;
                }
                case TAG_forwardingReason -> {
                    if (!c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": forwardingReason is not primitive");
                    this.forwardingReason = ForwardingReason.getForwardingReason(MapBerSupport.readInt(c));
                }
                case TAG_basicServiceGroup -> {
                    if (c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": basicServiceGroup is primitive");
                    ExtBasicServiceCodeImpl bs = new ExtBasicServiceCodeImpl();
                    MapBerSupport.decodeExplicitChoice(c, bs);
                    this.basicServiceGroup = bs;
                }
                case TAG_networkSignalInfo -> {
                    if (c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": networkSignalInfo is primitive");
                    ExternalSignalInfoImpl nsi = new ExternalSignalInfoImpl();
                    MapBerSupport.decodeNested(c, nsi);
                    this.networkSignalInfo = nsi;
                }
                case TAG_camelInfo -> {
                    if (c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": camelInfo is primitive");
                    CamelInfoImpl ci = new CamelInfoImpl();
                    MapBerSupport.decodeNested(c, ci);
                    this.camelInfo = ci;
                }
                ...
                case TAG_alertingPattern -> {
                    if (!c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": alertingPattern is not primitive");
                    AlertingPatternImpl ap = new AlertingPatternImpl();
                    MapBerSupport.decodeNested(c, ap);
                    this.alertingPattern = ap;
                }
                case TAG_ccbsCall -> {
                    MapBerSupport.readNull(c);
                    this.ccbsCall = true;
                ...
                case TAG_supportedCCBSPhase -> {
                    if (!c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": supportedCCBSPhase is not primitive");
                    this.supportedCCBSPhase = MapBerSupport.readInt(c);
                }
                case TAG_additionalSignalInfo -> {
                    if (c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": additionalSignalInfo is primitive");
                    ExtExternalSignalInfoImpl asi = new ExtExternalSignalInfoImpl();
                    MapBerSupport.decodeNested(c, asi);
                    this.additionalSignalInfo = asi;
                }
                case TAG_istSupportIndicator -> {
                    if (!c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": istSupportIndicator is not primitive");
                    this.istSupportIndicator = ISTSupportIndicator.getInstance(MapBerSupport.readInt(c));
                }
                ...
                case TAG_callDiversionTreatmentIndicator -> {
                    if (!c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": callDiversionTreatmentIndicator is not primitive");
                    CallDiversionTreatmentIndicatorImpl cdti = new CallDiversionTreatmentIndicatorImpl();
                    MapBerSupport.decodeNested(c, cdti);
                    this.callDiversionTreatmentIndicator = cdti;
                }
                ...
                case TAG_basicServiceGroup2 -> {
                    if (c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": basicServiceGroup2 is primitive");
                    ExtBasicServiceCodeImpl bs2 = new ExtBasicServiceCodeImpl();
                    MapBerSupport.decodeExplicitChoice(c, bs2);
                    this.basicServiceGroup2 = bs2;
                }
                case TAG_networkSignalInfo2 -> {
                    if (c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": networkSignalInfo2 is primitive");
                    ExternalSignalInfoImpl nsi2 = new ExternalSignalInfoImpl();
                    MapBerSupport.decodeNested(c, nsi2);
                    this.networkSignalInfo2 = nsi2;
                }
                case TAG_suppressMTSS -> {
                    if (!c.isPrimitive())
                        throw new AsnException(_PrimitiveName + ": suppressMTSS is not primitive");
                    SuppressMTSSImpl sm = new SuppressMTSSImpl();
                    MapBerSupport.decodeNested(c, sm);
                    this.suppressMTSS = sm;
                }
        ...
        if (this.msisdn == null)
            throw new AsnException(_PrimitiveName + ": missing msisdn");
        if (this.mapProtocolVersion >= 3 && (this.interrogationType == null || this.gmscAddress == null))
            throw new AsnException(_PrimitiveName + ": missing V3 mandatory field");
    }
    ...
    public void encodeData(AsnOutputStream asnOutputStream) throws MAPException {
        ...
        try {
            ...
            if (this.numberOfForwarding != null)
                asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, TAG_numberOfForwarding, this.numberOfForwarding);

            if (this.mapProtocolVersion >= 3) {
                if (this.interrogationType != null)
                    asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, TAG_interrogationType, this.interrogationType.getCode());

                if (this.orInterrogation)
                    asnOutputStream.writeNull(Tag.CLASS_CONTEXT_SPECIFIC, TAG_orInterrogation);

                if (this.orCapability != null)
                    asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, TAG_orCapability, this.orCapability);
                ...
                if (this.forwardingReason != null)
                    asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, TAG_forwardingReason, this.forwardingReason.getCode());

                if (this.basicServiceGroup != null) { // explicit tag encoding
                    asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, TAG_basicServiceGroup);
                    int pos = asnOutputStream.StartContentDefiniteLength();
            ...
            if (this.networkSignalInfo != null)
                ((ExternalSignalInfoImpl) this.networkSignalInfo).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC,
                        TAG_networkSignalInfo);

            if (this.mapProtocolVersion >= 3) {
                if (this.camelInfo != null) {
                    ((CamelInfoImpl) this.camelInfo).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC, TAG_camelInfo);
                }

                if (this.suppressionOfAnnouncement)
                    asnOutputStream.writeNull(Tag.CLASS_CONTEXT_SPECIFIC, TAG_suppressionOfAnnouncement);
                ...
                if (this.alertingPattern != null)
                    ((AlertingPatternImpl) this.alertingPattern).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC,
                            TAG_alertingPattern);

                if (this.ccbsCall)
                    asnOutputStream.writeNull(Tag.CLASS_CONTEXT_SPECIFIC, TAG_ccbsCall);

                if (this.supportedCCBSPhase != null)
                    asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, TAG_supportedCCBSPhase, this.supportedCCBSPhase);

                if (this.additionalSignalInfo != null)
                    ((ExtExternalSignalInfoImpl) this.additionalSignalInfo).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC,
                            TAG_additionalSignalInfo);

                if (this.istSupportIndicator != null)
                    asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, TAG_istSupportIndicator, this.istSupportIndicator.getCode());

                if (this.prePagingSupported)
                    asnOutputStream.writeNull(Tag.CLASS_CONTEXT_SPECIFIC, TAG_prePagingSupported);

                if (this.callDiversionTreatmentIndicator != null) {
                    ((CallDiversionTreatmentIndicatorImpl) this.callDiversionTreatmentIndicator).encodeAll(asnOutputStream,
                            Tag.CLASS_CONTEXT_SPECIFIC, TAG_callDiversionTreatmentIndicator);
                }

                if (this.longFTNSupported)
                    asnOutputStream.writeNull(Tag.CLASS_CONTEXT_SPECIFIC, TAG_longFTNSupported);

                if (this.suppressVtCSI)
                    asnOutputStream.writeNull(Tag.CLASS_CONTEXT_SPECIFIC, TAG_suppress_VT_CSI);

                if (this.suppressIncomingCallBarring)
                    asnOutputStream.writeNull(Tag.CLASS_CONTEXT_SPECIFIC, TAG_suppressIncomingCallBarring);

                if (this.gsmSCFInitiatedCall)
                    asnOutputStream.writeNull(Tag.CLASS_CONTEXT_SPECIFIC, TAG_gsmSCFInitiatedCall);

                if (this.basicServiceGroup2 != null) { // explicit tag encoding
                    asnOutputStream.writeTag(Tag.CLASS_CONTEXT_SPECIFIC, false, TAG_basicServiceGroup2);
                    int pos = asnOutputStream.StartContentDefiniteLength();
                ...
                if (this.networkSignalInfo2 != null)
                    ((ExternalSignalInfoImpl) this.networkSignalInfo2).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC,
                            TAG_networkSignalInfo2);

                if (this.suppressMTSS != null)
                    ((SuppressMTSSImpl) this.suppressMTSS).encodeAll(asnOutputStream, Tag.CLASS_CONTEXT_SPECIFIC, TAG_suppressMTSS);

                if (this.mtRoamingRetrySupported)
                    asnOutputStream.writeNull(Tag.CLASS_CONTEXT_SPECIFIC, TAG_mtRoamingRetrySupported);

                if (this.callPriority != null)
                    asnOutputStream.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, TAG_callPriority, this.callPriority.getCode());
            }
