...
public class ConnectionFlowControlTest extends SccpHarnessExt {
...
//        this.saveLogFile("aaaa.log");
    ...
    public void testBigCreditTwoDirections(boolean onlyOneStack) throws Exception {
        ...
        starter.lock();
        SenderThread sender1 = new SenderThread(starter, conn1, 1, sccpStack1);
        SenderThread sender2 = new SenderThread(starter, conn2, 2, sccpStack2);
...
//        }
        ...
        while (u1.getReceivedData().size() < 382 || u2.getReceivedData().size() < 382) {
            Thread.sleep(500);
        }

        assertEquals(u1.getReceivedData().size(), 382);
        assertEquals(u2.getReceivedData().size(), 382);
    ...
    public static class SenderThread implements Runnable {
        ...
        public SenderThread(ReentrantLock starter, SccpConnection conn, int workerNumber, SccpStack sccpStack) {
    ...
    private class SccpConnectionWithFlowControlImplProxy extends SccpConnectionWithFlowControlImpl {
        ...
        protected SccpFlowControl newSccpFlowControl(Credit credit) {
            return new SccpFlowControlProxy(stack.getName(), credit.getValue(), connectionLock, akAutoSending);
        }
