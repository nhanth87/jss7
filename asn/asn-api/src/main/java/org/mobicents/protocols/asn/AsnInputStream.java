...
public class AsnInputStream extends InputStream {
	...
	public AsnInputStream( byte[] buf ) {
		BerCursor.recordDecode();   // telemetry: legacy root decode op
		this.buffer = buf;
	...
	public double readRealData(int length) throws AsnException, IOException {
		...
		if ((infoBits & 0xC0) == 0) {
			...
			long bits = ((long) (doubleRep[0] & 0xFF) << 56) | ((long) (doubleRep[1] & 0xFF) << 48)
					| ((long) (doubleRep[2] & 0xFF) << 40) | ((long) (doubleRep[3] & 0xFF) << 32)
					| ((long) (doubleRep[4] & 0xFF) << 24) | ((long) (doubleRep[5] & 0xFF) << 16)
					| ((long) (doubleRep[6] & 0xFF) << 8) | ((long) (doubleRep[7] & 0xFF));
